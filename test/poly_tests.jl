...
@testset "输出测试" begin
    @test repr(MIME("text/plain"), Polynomial([1, 2, 0, 1])) == "Polynomial(x^3+2x^2+1)"
    @test repr(MIME("text/plain"), Polynomial([1, 4])) == "Polynomial(x+4)"
end
...
@testset "求导测试" begin
    ...
    @test ∂(Polynomial([1.])) != Polynomial([0])
    @test ∂(Polynomial([1, 2, -3])) == Polynomial([2, 2])
end
